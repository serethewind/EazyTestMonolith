...
public class QuestionController {
    ...
    @PostMapping
    public ResponseEntity<ReadQuestionResponseDto> createQuestion(@RequestBody QuestionRequestDto questionRequestDto) {
        return new ResponseEntity<>(questionService.createSingleQuestionInstance(questionRequestDto), HttpStatus.CREATED);
    }

    @PostMapping("batch")
    public ResponseEntity<ReadQuestionResponseDto> createMultipleQuestions(@RequestBody List<QuestionRequestDto> questionRequestDtoList){
        return new ResponseEntity<>(questionService.createMultipleQuestion(questionRequestDtoList), HttpStatus.CREATED);
    }

    @GetMapping
    public ResponseEntity<ReadQuestionResponseAlternativeDto> findAllQuestions(@RequestParam(value = "pageNo", defaultValue = "0", required = false) int pageNo, @RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize) {
...
//    }
    ...
    @GetMapping("{id}")
    public ResponseEntity<ReadQuestionResponseDto> findQuestionById(@PathVariable("id") Long questionId) {
        return new ResponseEntity<>(questionService.findQuestionById(questionId), HttpStatus.OK);
    }

    @GetMapping("category-filter")
    public ResponseEntity<ReadQuestionResponseAlternativeDto> findQuestionsByCategory(@RequestParam("category") String category, @RequestParam(value = "pageNo", defaultValue = "0", required = false) int pageNo, @RequestParam(value = "pageSize", defaultValue = "20", required = false) int pageSize
    ) {
        return new ResponseEntity<>(questionService.findQuestionsByCategory(category, pageNo, pageSize), HttpStatus.OK);
    }

    @GetMapping("search")
    public ResponseEntity<ReadQuestionResponseAlternativeDto> findQuestionsBySearchQuery(@RequestParam("search-word")String searchWord, @RequestParam(value = "pageNo", defaultValue = "0", required = false) int pageNo, @RequestParam(value = "pageSize", defaultValue = "8", required = false) int pageSize){
        return new ResponseEntity<>(questionService.findQuestionBySearchQuery(searchWord, pageNo, pageSize), HttpStatus.OK);
    }

    @PutMapping("{id}/update")
    public ResponseEntity<ReadQuestionResponseDto> updateQuestion(@RequestBody QuestionRequestDto questionRequestDto, @PathVariable("id") Long questionId) {
        return new ResponseEntity<>(questionService.updateQuestionInstance(questionId, questionRequestDto), HttpStatus.OK);
    }


    @DeleteMapping("{id}")
    public ResponseEntity<ReadQuestionResponseDto> deleteQuestionById(@PathVariable("id") Long questionId){
        return new ResponseEntity<>(questionService.deleteQuestionById(questionId), HttpStatus.OK);
    }

    @PutMapping("{id}/reactivate")
    public ResponseEntity<ReadQuestionResponseDto> reactivateQuestion(@PathVariable("id") Long questionId){
